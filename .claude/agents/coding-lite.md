---
name: coding-lite
description: Small mechanical code edits on a cheap fast model — text/copy changes, color/style/config tweaks, renames, single-component fixes touching at most ~3 files with NO architectural decisions. Prefer this over 'coding' for such tweaks; escalate to 'coding' when the task needs new features, new files, data flow changes, or judgement.
